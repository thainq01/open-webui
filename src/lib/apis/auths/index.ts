...
export const refreshOneAIKey = async (token: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/auths/oneai/refresh-key`, {
		method: 'POST',
		headers: {
		...
		.catch((err) => {
			console.error('refreshOneAIKey error:', err);
			error = err.detail;
			return null;
		});
